...
export default {
  ...
  computed: {
    selectedChild() {
      return this.nodeSelectedObject &&
        this.nodeSelectedObject.children &&
        this.nodeSelectedObject.children.length
        ? this.nodeSelectedObject.children.filter((e) => e.selected).length
        : null;
    },
    modeSelected() {
      return this.nodeSelectedObject.selected
        ? this.nodeSelectedObject.selected
        : false;
    },
  },
  watch: {
    selectedChild(nv, ov) {
      this.$emit(
        "passStateToModel",
        this.nodeSelectedObject.children.filter((e) => e.selected),
    ...
    modeSelected(nv, ov) {
      if (nv === true) {
        this.nodeSelectedObject.children.forEach((element) => {
          element.selected = true;
        });
        this.$emit(
          "passStateToModel",
          this.nodeSelectedObject.children.map((e) => {
            return {
              ...e,
              selected: true,
            };
